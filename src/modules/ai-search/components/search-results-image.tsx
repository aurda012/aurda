      ...
      {images.slice(0, 5).map((image: any, index: number) => (
        <Dialog key={index}>
          <DialogTrigger asChild>
            <div
              className="relative aspect-video w-[calc(50%-0.5rem)] cursor-pointer md:w-[calc(33%-0.5rem)]"
            ...
            >
              ...
              {index === 3 && images.length > 4 && (
                <div className="absolute inset-0 flex items-center justify-center rounded-md bg-black/30 text-sm text-white/80 md:hidden">
                  <PlusCircle size={24} />
                </div>
              )}
            </div>
